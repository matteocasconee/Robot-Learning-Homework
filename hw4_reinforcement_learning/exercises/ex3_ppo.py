...
class PPOAgent:
    ...
    def compute_surrogate_loss(self, logp_batch, old_logp_batch, adv_batch):
        ...
        clipped_ratio = torch.clamp(ratio, min= 1 - self.clip_ratio, max= 1 + self.clip_ratio)
    ...
    def compute_value_loss(self, val_batch, old_val_batch, ret_batch):
        ...
        value_clipped = old_val_batch+torch.clamp(val_batch-old_val_batch,min=-self.clip_ratio,max=self.clip_ratio)
    ...
    def update(self, rollout_batch) -> PPOUpdateStats:
        ...
        for mini_batch in self.mini_batch_generator(rollout_batch):
            ...
            self.learning_rate = self.adjust_learning_rate(kl.item(), self.learning_rate)
