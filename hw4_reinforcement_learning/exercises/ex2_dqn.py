...
class DQN:
    ...
    def take_action(self, state):
        ...
        state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(self.device) #Exploitation
        with torch.no_grad():    
            q_val=self.q_net(state_tensor)
    ...
    def update(self, transition_dict):
        ...
        with torch.no_grad():
            ...
            target_net=self.q_net(next_states).max(dim=1,keepdim=True)[0]
            q_targets = rewards+self.gamma*target_net*(1-dones)
