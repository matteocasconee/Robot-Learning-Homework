...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    """
    space=np.linspace(0,2*np.pi,count,endpoint=False)
    y,z=get_lemniscate_keypoint(space,width)
    xcolumn=np.full(y.size,x_offset)
    return np.array([xcolumn,y,z+z_offset]).T
...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        weighted_err=np.concatenate((err_pos*pos_gain, np.zeros(3))) 
