...
def reset_robot(default_qpos: np.ndarray) -> np.ndarray:
    ...
    """
    return default_qpos+np.random.uniform(-0.5,0.5,size=default_qpos.shape)
...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    """
    lower_bounds=jnt_range[:,0]
    upper_bounds=jnt_range[:,1]
    midpoints=(upper_bounds+lower_bounds)/2
    range=(upper_bounds-lower_bounds)/2
    target_qpos=midpoints+action*range
    return target_qpos
...
def compute_reward(
    ...
    # action_penalty = 0.008 * np.sum(np.square(action))

    # return dense_reward+sparse_reward-vel_penalty-action_penalty

    #LOG 11
    
    if qvel is not None and action is not None:
        dense_reward = np.exp(-5.0 * ee_tracking_error)
        sparse_reward = 1.0 if ee_tracking_error < 0.005 else 0.0
        reward=dense_reward+sparse_reward
        vel_penalty = 0.005 * np.mean(np.square(qvel))
        action_penalty = 0.01 * np.mean(np.square(action))
        penalty_factor = np.exp(-vel_penalty - action_penalty)
        return float(reward * penalty_factor)
    #     dense_reward = np.exp(-5.0 * ee_tracking_error)
    #     vel_penalty = 0.005 * np.mean(np.square(qvel))
    #     action_penalty = 0.01 * np.mean(np.square(action))
    #     penalty_factor = np.exp(-vel_penalty - action_penalty)
    #     return float(dense_reward * penalty_factor)
    else:
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray:
    ...
    ee_quat_base=base_rot_w.T @ ee_rot_w
    ee_quat_base_normalized=quat_normalize(rot_mat_to_quat(ee_quat_base))
    target_pos_base=base_rot_w.T@(target_pos_w-base_pos_w)

    obs=np.concatenate([qpos,ee_pos_base,ee_quat_base_normalized,target_pos_base])
    return obs